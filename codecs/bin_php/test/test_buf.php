...
test\step( $iter->write_next(), "write_next failed" );

echo "   -- Testing of writin 3byte length of tag \n";
$val_u32 = 0x87654321;
$expect = "\x0a\x04\x78\x56\x34\x12";
...
test\step( $iter->decode_next(), "decode_next failed" );
...
test\step( $iter->decode_to_next(), "decode to next failed.");
test\step( $iter->tag() == 2, "the very first tag must have been found.");
test\step( $iter->decode_to_end(), "decode_to_eoscope must return true on EOF.");
test\step( ! $iter->is_complete(), "the iter shall not be complete. ");
test\step( $iter->is_eof(), "the item shall point to EOF");
test\step( $iter->decode_to_end(), "decode_to_eoscope must return true on EOF second attempt.");
...
$iter->copy_from( $iter_ini );
test\step( $iter->decode_to_stuffing(), "stuffing was not found but it is there");
...
test\step( ! $iter->decode_to_tag( 333 ), "1- the tag 333 was found but we shall not have it." );
test\step( $iter->is_eof(), "the item shall point to EOF");
$iter->copy_from( $iter_ini );
test\step( ! $iter->decode_to_tag( 333 ), "2- the tag 333 was found but we shall not have it." );
test\step( $iter->is_eof(), "the item shall point to EOF");
...
$iter->copy_from( $iter_ini );
test\step( $iter->decode_to_end(), "tausch_decode_to_eoscope must return true on EOF.");
$val_bool = false;
...
$iter->copy_from( $iter_ini );
test\step( $iter->decode_to_next(), "Iterating must have passed");
...
test\step( ! $iter->decode_to_next(), "Iterating must have failed");
...
test\step( $iter->decode_to_stuffing(), "EOF must result true when decoding to stuffing");
test\step( $iter->is_eof(), "the item shall point to EOF");
...
echo "   -- BLOB\n";
test\step( !$iter->decode_next(), "decode next must have been failed because of EOF");
...
test\step( $iter->write_next(), "preparing to write next failed");
...
$iter->copy_from( $iter_ini );
test\step( $iter->decode_to_tag(40), "finding first blob item failed" );
test\step( $iter->decode_to_tag(40), "finding second blob item failed" );
...
test\step( $iter->decode_to_tag(41), "finding UTF-8 item failed");
...
test\step( ! $iter->decode_next(), "iterating out of EOF must fail" );
test\step( $iter->vlen() == 0, "iter value len must be 0" );
...
test\step( $iter->write_next(), "advancing to next failed" );
...
$iter->copy_from( $iter_ini );
test\step( $iter->decode_to_tag(100), "finding tag 100 failed" );
//echo bin2hex( substr( $iter->message(), $iter->idx(), 30) ) . "\n";
test\bincomp( substr( $iter->message(), $iter->idx(), 30), "\x91\x03\x06\x04\x1d\x00\x00\x00\x03\x96\x03\x04\x1d\x00\x00\x00\x07" );
test\step( $iter->decode_to_tag(1), "finding tag 100/1 failed" );
test\step( $iter->decode_to_end(), "finding end failed" );
...
$iter->copy_from( $iter_ini );
test\step( $iter->decode_to_tag(100), "finding tag 100 failed" );
test\step( ! $iter->decode_to_tag(101), "finding tag 100/101 must fail" );
...
$iter->copy_from( $iter_ini );
test\step( $iter->decode_to_tag(100), "finding tag 100 failed" );
test\step( ! $iter->decode_to_stuffing(), "finding stuffing must fail" );
//echo " tag is " . $iter->tag() . " \n";
test\step( $iter->tag() == 101, "Iter shall stop on next element after eos" );
test\step( $iter->decode_to_end(), "decoding to eof failed" );

echo " ## Various \n";
echo "   -- decoding over buffer edge\n";
$buffer_over = "\x0a\x14\x00\x00\x00";
$over = new tauschema\Iterator($buffer_over);
test\step( ! $over->decode_next(), "1; over the edge decode must fail" );
// throws test\step( $over->tag() == -1, "1; tag must be -1" );
$buffer_over = "\x0a\x04\x00\x00\x00\x07";
$over = new tauschema\Iterator($buffer_over);
test\step( $over->decode_next(), "2; the first item shall be ok" );
test\step( $over->tag() == 2, "2; tag must be 2" );
test\step( ! $over->decode_next(), "2; over the edge decode must fail" );
